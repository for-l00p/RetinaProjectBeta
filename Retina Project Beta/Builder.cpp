...
std::array<double, 18> rodIntervals{ 0, 0.0536332, 1.0166, 1.99218, 2.99914, 4.07128, 5.00363, 5.97013, 6.97032, 7.97061, 8.93607, 9.93635, 10.9365, 11.9365, 12.9713, 13.9023, 14.8679, 15.868 };
...
std::array<double, 18> rodDensity{ 0,0.0148568, 68.9053, 107.757, 132.022, 138.163, 136.795, 133.659, 126.543, 119.869, 111.87, 105.196, 98.0802, 90.0802, 82.9626, 75.4076, 68.2931, 60.7351 };
...
Neuron* build(int type) {//0 is rod, 1 is cone, 2 is ganglion
	...
	if (type == 0) {
		n = new Rod(loc);
		outfile << n->getPoint().x << "\t" << n->getPoint().y << "\n";
	}
	else if (type == 1) {
		double coneType = ((double)rand() / (RAND_MAX));
		if (coneType < probabilityRED) {
			n = new Cone(Cone::RED, loc);
			outfile << n->getPoint().x << "\t" << n->getPoint().y << "\t" << "red" << "\t" << "#FF0000" << "\n";
		}
		else if (coneType < probabilityGREEN + probabilityRED) {
			n = new Cone(Cone::GREEN, loc);
			outfile << n->getPoint().x << "\t" << n->getPoint().y << "\t" << "green" << "\t" << "#008000" << "\n";
		}
		else {
			n = new Cone(Cone::BLUE, loc);
			outfile << n->getPoint().x << "\t" << n->getPoint().y << "\t" << "blue" << "\t" << "#3399FF" << "\n";
		}
	}
	else {
		n = new Ganglion(loc);
		std::cout << n->getPoint().x;
		outfile << n->getPoint().x << "\t" << n->getPoint().y << "\n";
...
int main()
{
	...
	outfile.open("outputGCells.txt", std::ofstream::out | std::ofstream::trunc);
	rvalues.open("Gvalues.txt", std::ofstream::out | std::ofstream::trunc);
